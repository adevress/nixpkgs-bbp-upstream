...
# On powerpc64, it is impossible to use patchelf to patch patchelf
# without copying this one first
...
export PATH=$out/bin
cat $out/lib/libc.so | sed "s|/a*/e*-[^/]*/|$out/|g" > $out/lib/libc.so.tmp
mv $out/lib/libc.so.tmp $out/lib/libc.so
cat $out/lib/libpthread.so | sed "s|/a*/e*-[^/]*/|$out/|g" > $out/lib/libpthread.so.tmp
