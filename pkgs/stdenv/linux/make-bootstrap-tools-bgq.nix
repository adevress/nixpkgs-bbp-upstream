...
with import ../../../../default.nix {inherit system;};
...
rec {
  ...
  build =
	...
	cp -d ${glibc}/bin/getconf $out/bin
	...
	if [[ "$(uname -m)" == "ppc64" ]]; then
		pushd $out/lib
		local LIB_VERSION="2.21"
		ln -s ld64.so.1 ld-''${LIB_VERSION}.so || true
		ln -s ld-''${LIB_VERSION}.so ld-''${LIB_VERSION}.so.2 || true
		popd
	fi
